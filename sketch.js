...
function getAmplitudeVariance(x, y) {
  const xoff = x / 30 - frameCount / 150 + y;
  return noise(xoff);
}

function mouseMoved() {
   const _mouseX = constrain(mouseX, 0, width);
   const _mouseY = constrain(mouseY, 0, height);
  
   xVertex0 = _mouseX;
   mouseXFromCenter = _mouseX - width / 2;
   maxAmplitude = abs(_mouseY - height / 2);
